...
string alu(string code)
{
    ...
    // To generate ALU OP code used by execute()
...
void Writeback(int rg, int result)
{
	...
	// Last step of an instruction excecution
    if (regWrite == 0 && memWrite == 1)
    { // SW
        d_mem[rg] = result;
...
void Mem(int rg, int address)
{
    ...
    if (memWrite == 1){ //SW
        ...
        val = d_mem[address / 4]; // diivide by 4 to place it in the memory by 4
        Writeback(rg, val);
    }else{
        // R/I
...
{
    // Executes computations with ALU. Register values and sign-extended offset values retrieved/computed by Decode() function used for computation
	// Recieves 4-bit alu_op input and run operations
...
{
    // Decodes an instruction fetched by Fetch()
